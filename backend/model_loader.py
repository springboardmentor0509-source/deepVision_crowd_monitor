...
import joblib
import numpy as np
from typing import cast, Tuple, Optional, Union, Any
...
from src.csrNet_Model.csr_model import CSRNet
from src.mobile_csrnet.mobile_csr_model import MobileNetCSRNet
from src.random_forest.preprocessing_rf import extract_features
...
class ModelManager:
    ...
    def load_model(self, model_name: str) -> Union[torch.nn.Module, Any]:
        """Load and cache model by name. Returns loaded model object (torch.nn.Module or sklearn model)."""
    ...
    def predict(self, model_name: str, image_path: str) -> Tuple[Optional[np.ndarray], float]:
        """
        ...
        if model_name == "RandomForest":
            ...
            # model may be a sklearn estimator; cast to Any to satisfy type checker
            rf_model = cast(Any, model)
            count = rf_model.predict(features)[0]
            return None, float(count)
        img = Image.open(image_path).convert("RGB")
        # Use cast to tell Pylance that transform returns a Tensor
        tensor = cast(torch.Tensor, self.transform(img)).unsqueeze(0).to(self.device)
